...
/*   Updated: 2021/01/04 16:46:54 by gbouwen       ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void	expand_files(t_data *data, t_node *node)
{
	...
	if (node->type == FILE_IN && data->expand_error != 1)
		check_if_file_exists(data, list->content);
	if ((node->type == FILE_OUT || node->type == FILE_OUT_APPEND) && data->expand_error != 1)
		open_or_create_file(data, list->content);
	expand_files(t_data *data, node->right);
	expand_files(t_data *data, node->left);
}
